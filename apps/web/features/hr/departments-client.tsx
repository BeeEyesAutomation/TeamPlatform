...
import { deleteDepartment, fetchDepartments } from "./hr-api";
...
export function DepartmentsClient() {
  ...
  async function load() {
    try {
      const response = await fetchDepartments({ search, status });
      setItems(response.data.items);
      setError("");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Cannot load departments");
    }
  }
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Phong ban</h1>
          <p className="text-sm text-muted">Quan ly danh muc phong ban.</p>
        </div>
        {canManage ? (
          <Link className="rounded-md bg-primary px-4 py-2 text-sm font-semibold text-white" href="/departments/new">
            Tao phong ban
          </Link>
        ) : null}
      </div>
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="bg-surface text-left text-muted">
            <tr>
              ...
              <th className="p-3">Ten phong ban</th>
                ...
                <td className="p-3 text-right">
                  {canManage ? (
                    <div className="flex justify-end gap-2">
                      <Link className="text-primary" href={`/departments/${item.id}/edit`}>
                        Sua
                      </Link>
